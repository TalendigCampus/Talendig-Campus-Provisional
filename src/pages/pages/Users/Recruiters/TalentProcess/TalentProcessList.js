...
function EnhancedTable() {
  ...
  const rows = recruiterTalents.filter(
    (talents) =>
      talents?.recruiterProcess?.activeProcess &&
      !talents?.recruiterProcess?.decision?.selection &&
      !talents?.recruiterProcess?.decision?.comments
  );
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          ...
          <RecruiterTalentDialog
            actionToPerform={RECRUITER_ACTION_TYPE.removeFromProcess}
          />
          <UndoAction
            actionToPerform={RECRUITER_ACTION_TYPE.redoRemoveFromProcess}
          />
        </Grid>
      </Grid>
    </React.Fragment>
