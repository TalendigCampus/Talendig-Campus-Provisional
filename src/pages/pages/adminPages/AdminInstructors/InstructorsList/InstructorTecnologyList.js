...
function DataGridDemo() {
  ...
  const filterTecnologies = () => {
    ...
    for (let i = 0; i < instructor.technology.length; i++) {
      tecnologies = tecnologies.filter(
        (tecnology) => tecnology.id !== instructor.technology[i]
      );
    }
  ...
  React.useEffect(() => {
    setRows(
      instructor.technology.map((tecnoId, index) => ({
        id: ++index,
        tecnoId,
